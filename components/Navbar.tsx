      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-3 md:py-0 md:h-20 flex flex-col md:flex-row items-center justify-between gap-3 md:gap-6">
        ...
        <div className="w-full md:w-auto flex items-center justify-between">
          ...
          <div className="flex items-center gap-2 md:hidden">
            <Link href="/wishlist" className="p-1.5 text-rose-500 font-bold text-sm">
              ♥ <span className="text-xs">({isMounted ? wishlist.length : 0})</span>
            </Link>
            
            <Link
              href="/cart"
              className="bg-blue-600 text-white px-2.5 py-1.5 rounded-xl text-xs font-bold cursor-pointer"
            >
            ...
            <button
              ...
              className="p-2 text-slate-700 hover:text-blue-600 focus:outline-none rounded-lg border border-slate-200 ml-1"
        ...
        {isMenuOpen && (
          <div className="w-full md:hidden flex flex-col gap-3 pt-3 pb-2 border-t border-slate-100 animate-in fade-in slide-in-from-top-2">
            <nav className="flex flex-col gap-2 font-bold text-xs text-slate-700 px-1">
              <Link
                href="/"
                onClick={() => setIsMenuOpen(false)}
                className="hover:text-blue-600 py-1.5 px-2 rounded-lg hover:bg-slate-50"
              >
                🏠 Home
              </Link>
              <Link
            ...
            <div className="pt-2 border-t border-slate-100 flex items-center justify-between px-1">
              {isMounted && user ? (
                <div className="flex items-center justify-between w-full bg-slate-100 p-2 rounded-xl border border-slate-200">
                ...
                <Link
                  href="/login"
                  onClick={() => setIsMenuOpen(false)}
                  className="w-full text-center text-xs font-bold text-blue-600 border border-blue-600/30 hover:bg-blue-50 py-2 rounded-xl transition-colors"
                >
                  Sign In
                </Link>
              )}
