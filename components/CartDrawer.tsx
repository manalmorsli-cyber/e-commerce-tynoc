        ...
        <div>
          ...
          {cart.length === 0 ? (
            <div className="text-center py-20 px-4 flex flex-col items-center justify-center h-full">
              <div className="w-20 h-20 bg-slate-50 text-slate-400 rounded-full flex items-center justify-center mb-6 text-3xl border border-slate-100 shadow-sm">
                🛒
              </div>
              <h3 className="text-lg font-bold text-slate-900 mb-2">Your cart is empty</h3>
              <p className="text-slate-500 text-xs leading-relaxed max-w-[250px] mx-auto mb-8">
                Looks like you haven't added any products to your cart yet.
              </p>
              <button
                onClick={() => setIsCartOpen(false)}
                className="bg-blue-600 hover:bg-blue-700 text-white font-bold px-8 py-3.5 rounded-xl text-xs transition-all shadow-md shadow-blue-600/20 cursor-pointer"
              >
                Start Shopping
              </button>
            </div>
        ...
        {cart.length > 0 && (
          <div className="pt-4 border-t border-slate-100 space-y-2.5">
            ...
            <Link
              href="/cart"
              onClick={() => setIsCartOpen(false)}
              className="block w-full bg-slate-100 hover:bg-slate-200 text-slate-800 font-bold py-3 rounded-xl text-center text-xs transition-all cursor-pointer"
            >
              View Full Cart
            </Link>
